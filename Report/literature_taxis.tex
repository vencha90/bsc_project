...
TRANSIMS \parencite{Smith1995taxi+transims} and EMME/2
\parencite{Gao2010taxi+comparison} aggregate. TRANSIMS is limited to certain
modules and specific inputs and ways of operating, not supporting agents.
EMME/2 older software, conventional, top-down approach so not suitable for
agents.

\textcite{Bernhardt2007taxi+agent} gives an overview of agent-based software
for traffic modeling, naming MATSim as the most useful of these. MATSim is the
most noteworthy agent-based simulation and in active development. Very
extensive capabilities and modules, but complex to use and extend. MATSim
covered by \textcite{Balmer2008taxi+matsim}.

\textcite[78]{Bernhardt2007taxi+agent} cites Bonabeau(2002a) for the following conditions when agent based modelling is appropriate:
...
environment; agents with their types, attributes, allowable and initial values for the attributes;
agent interaction rules with each other and the environment.
